...
public sealed class EncryptedSourceGenerationViewModel : ViewModelBase
{
    private readonly IEncryptedSourceFileGenerator _generator;
    private readonly IGeneratedSourceFilePathProvider _pathProvider;
    private AlgorithmOption? _selectedAlgorithm;
    private string _sourceCode = string.Empty;
    private bool _isGenerating;

    public EncryptedSourceGenerationViewModel(
        IEncryptedSourceFileGenerator generator,
        IGeneratedSourceFilePathProvider pathProvider)
    {
        ...
        GenerateCommand.ThrownExceptions.Subscribe(ex => Logger.Error("生成加密 C# 文件时发生异常。", ex));
    ...
    public string OutputDirectory => _pathProvider.SourceFileDirectory;
    ...
    private async Task GenerateAsync()
    {
        ...
        try
        {
            Logger.Info("开始生成加密 C# 类文件。");
            var result = await _generator.GenerateAsync(SourceCode);
            ...
            if (!result.Succeeded)
            {
                Logger.Error("加密 C# 文件生成失败。");
                return;
            }

            Logger.Info($"加密 C# 文件生成成功：{result.OutputPath}");
            Logger.Debug($"生成目录：{OutputDirectory}");
        }
        finally
        {
            IsGenerating = false;
    ...
    private static string CreateSource(string className, string methodBody)
    {
        ...
        namespace EncryptedGeneratedSources;
