...
✅ **成功**：这是最推荐的方案，简单可靠，只需要确保不同平台的库文件名统一（Linux下去掉lib前缀）。
...
| 静态加载 | 多工程 + 条件编译           | 发布前通过脚本全局设置宏                           | ✅ **成功（配合发布脚本）**   | 推荐，需要不同平台库路径不同，但希望统一库文件名        |
| 静态加载 | 多工程 + 仅库名（无扩展名） | 类库只写库名 + csproj 条件复制（Linux去lib前缀）   | ✅ **跨平台完美成功**        | **最推荐**，简单可靠                                   |
